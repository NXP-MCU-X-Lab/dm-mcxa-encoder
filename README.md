...
- 上位机：当前 FreeMASTER 入口为 `freemaster/index.html`；V2 调试优先看 `v2_result.*`、`v2_diag.*` 和 `s_v2_cal_flat`。
...
当前 TSA 只暴露 A1/A2 四路 raw ADC 字段，不再保留 V1 单码道调试字段作为验收口径。
...
7. `fm_cal_done = 1` 后，重新观察 `v2_result.*` 和 `s_v2_cal_flat`。
...
- `s_v2_cal_flat[0..5]`：A1 `center_sin`、`center_cos`、`T[0,0]`、`T[0,1]`、`T[1,0]`、`T[1,1]`
- `s_v2_cal_flat[6..11]`：A2 `center_sin`、`center_cos`、`T[0,0]`、`T[0,1]`、`T[1,0]`、`T[1,1]`
- `s_v2_cal_flat[12]`：`phase_a1_zero_deg`
- `s_v2_cal_flat[13]`：`phase_a2_zero_deg`
...
V1 `encoder_result.*`、`fm_direction`、速度和多圈变量当前不作为 V2 阶段验收目标。
...
- 在线校准结果不写 flash，复位后恢复 `v2_encoder_calibration_set_board_defaults()` 中的固化默认值。
